...
fn renders_action_items_as_checkboxes_with_the_owner_when_known() {
    let md = render_summary_markdown(&full_result(), &meeting());
    assert!(md.contains("- [x] Grab a booth slot — Parker"));
    assert!(md.contains("- [x] Create a duties checklist\n"));
}
...
- [x] **Step 5: Verify the truncation fix against the real endpoint**
...
```rust
    ...
    fn write_summary_files_writes_the_rendered_notes_and_action_items() {
        ...
        assert!(summary_md.contains("- [x] Send follow-up email — Parker"));
...
- [x] **Step 5: Add the fields to the setup dialog**
...
- [x] **Step 1: Point the config at the benchmarked model**
...
- [x] **Step 2: Regenerate notes for the reference meeting**
...
- [x] **Step 3: Confirm the truncation fix on a long meeting**
...
- [x] **Step 4: Commit any config documentation updates**
